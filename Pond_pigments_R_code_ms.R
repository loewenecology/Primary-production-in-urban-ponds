...
##### Road salt pollution elevates risk of harmful cyanobacteria blooms #####
...
metals = read.table("Pond_metals_2022_ms.csv", header = T, sep = ",") #read in pond metals data
...
##################
# Conduct PCA of metals data
##################

# Set row names
metals <- column_to_rownames(metals, var = "Site")

# Conduct PCA of log transformed metals concentrations
PCA_metals <- rda(log(metals), center = TRUE, scale = TRUE)
summary(PCA_metals)

# Obtain PCA scores
metals_scores <- as.data.frame(scores(PCA_metals)$species)
site_scores <- as.data.frame(scores(PCA_metals)$sites)

# Create column for site name
site_scores <- rownames_to_column(site_scores, var = "Site")

# Simplify metals parameter names
rownames(metals_scores) <- gsub(x = rownames(metals_scores), pattern = "Total.", replacement = "")  

# Plot PCA of metals (9 x 9)
(plot_PCA_metals <- ggplot() +
    ...
    geom_point(data = metals_scores, aes(x = PC1, y = PC2), size = 3, color = "red") +
    geom_label_repel(data = metals_scores, aes(x = PC1, y = PC2, label = rownames(metals_scores)), size = 3, max.overlaps = Inf) +
    scale_x_continuous("PC1 (47%)", limits = c(-0.8, 4.4), breaks = c(-1, 0, 1, 2, 3, 4)) +
    scale_y_continuous("PC2 (14%)", limits = c(-1.9, 2.2), breaks = c(-2, -1, 0, 1, 2)) +
    workingtheme_pca +
    theme(legend.position = "none",
          panel.grid.major = element_blank()))

# Add metals PCA scores to site data
sites <- left_join(sites, site_scores, by = c("Site"))
sites <- column_to_rownames(sites, "Site")

# Fix column names
colnames(sites) <- gsub(x = colnames(sites), pattern = "\\.", replacement = " ")

# Clean up variable names
names(sites)[names(sites) == 'PC1'] <- 'Metal PC1'
...
##################
# Create variable correlation heatmap
##################
...
ggplot(data = env.cor.long, aes(x = var1, y = var2, fill = cor)) +
  geom_tile() +
  scale_fill_gradient(name = "Pearson Correlation Coefficient", low = "blue4", high = "red") +
...
(plot.PCA.pigs <- ggplot() +
    ...
    scale_y_continuous("PC2 (24%)", limits = c(-0.7, 0.7), breaks = c(-0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75)) +
    workingtheme_pca +
    theme(legend.position = "none",
          panel.grid.major = element_blank()))
...
site_select_phyto <- rownames_to_column(site_select_phyto, var = "Site")
site_select_peri <- rownames_to_column(site_select_peri, var = "Site")
...
# Plot PCA of pigments with environmental vectors (6 x 6)
...
# Plot PCA of taxonomic groups (6 x 6)
(plot.PCA.group <- ggplot() +
    geom_hline(yintercept = 0, linetype = "dashed", color = "lightgrey", linewidth = 0.5) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "lightgrey", linewidth = 0.5) +
    geom_line(data = site_score_delim, aes(x = PC1, y = PC2, group = site), color = "grey", linewidth = 1) +
    geom_point(data = site_score_delim, aes(x = PC1, y = PC2, color = type), size = 3) +
    geom_point(data = pig_scores, aes(x = PC1, y = PC2), color = "black", size = 3) +
    geom_label_repel(data = pig_scores, aes(x = PC1, y = PC2, label = rownames(pig_scores)), size = 3) +
    scale_x_continuous("PC1 (43%)", limits = c(-0.7, 1.5), breaks = c(-0.6, -0.3, 0, 0.3, 0.6, 0.9, 1.2, 1.5)) +
...
# Plot PCA of taxonomic groups with environmental vectors (6 x 6)
...
(Sampling_Mapa <- ggplot() +
    ...
    geom_sf(data = samplingLocs, size = 5,
            aes(color = Chloride)) +
...
(Sampling_Mapb <- ggplot() +
    annotation_map_tile(zoom = 8, "cartolight") +
    ...
    annotation_north_arrow(location = "bl", which_north = "true",
                           ...
                           style = north_arrow_fancy_orienteering) +
    xlab("Longitude") + ylab("Latitude") +
    coord_sf(xlim = c(-77.65205, -81.84537),
             ylim = c(41.615, 45.79379),
             crs = 4326))
...
pigmentord <- as.factor(c("Fucoxanthin",
                "Diadinoxanthin",
                "Diatoxanthin",
                "Alloxanthin",
                "Chlorophyll.b",
...
filter.pig.coef <- c(summary(fit.fuco.filter)$coefficients[2, 1],
                     summary(fit.diad.filter)$coefficients[2, 1],
...
filter.pig.se <- c(summary(fit.fuco.filter)$coefficients[2, 2],
                   summary(fit.diad.filter)$coefficients[2, 2],
...
filter.cl.allpigs$sig <- as.factor(c("P>0.10", #fit.fuco.filter
                                     "P>0.10", #fit.diad.filter
...
slide.pig.coef <- c(summary(fit.fuco.slide)$coefficients[2, 1],
                    summary(fit.diad.slide)$coefficients[2, 1],
...
slide.pig.se <- c(summary(fit.fuco.slide)$coefficients[2, 2],
                  summary(fit.diad.slide)$coefficients[2, 2],
...
slide.cl.allpigs$sig <- as.factor(c("P<0.05", #fit.fuco.slide
                                    "P>0.10", #fit.diad.slide
...
slide.cl.allpigs$pigmentord <- ordered(slide.cl.allpigs$pigmentord, levels = c("Fucoxanthin",
                                                                                "Diadinoxanthin",
                                                                                "Diatoxanthin",
                                                                                "Alloxanthin",
                                                                                "Chlorophyll.b",
...
(plot.cl.allpigs <- ggplot() +
    ...
    geom_errorbar(data = filter.cl.allpigs, width = 0.2, color = "darkgrey",
                  aes(x = pigmentord, ymin = coef - se, ymax = coef + se)) +
...
# Plot standardized chloride slope coefficients for taxonomic groups (6 x 3 portrait)
(plot.cl.alltaxa <- ggplot() +
    geom_hline(yintercept = 0, linetype = "dashed", color = "darkgrey", linewidth = 1) +
    geom_errorbar(data = slide.cl.alltaxa, width = 0.2, color = "grey",
                  aes(x = taxaord, ymin = coef - se, ymax = coef + se)) +
    geom_errorbar(data = filter.cl.alltaxa, width = 0.2, color = "darkgrey",
                  aes(x = taxaord, ymin = coef - se, ymax = coef + se)) +
...
(plot.cyano.all <- ggplot() +
    ...
    geom_errorbar(data = filter.cyano.all, width = 0.2, color = "darkgrey",
                  aes(x = covariateord, ymin = coef - se, ymax = coef + se)) +
    ...
    scale_y_continuous("Standardized Parital Slope Coefficient") +
